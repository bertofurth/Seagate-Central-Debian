...
Our tests showed that when both CPU cores are running at full load for
a period of over about a minute or so, spurious errors can be 
generated causing the system to fail. In particular, errors such as
"Segmentation Fault" or "Illegal Instruction" are reported.

If the default non SMP CPU is run at high CPU loads then the system seems to
run stably.
